...
const afterDeploy: AfterDeployInit = async (networkDeployInfo: INetworkDeployInfo) => {
    ...
    assert(networkName in BeanStalkProxyAddresses, "Need valid beanstalk address to interface with the contract");
    const beanStalkProxyAddress: string = BeanStalkProxyAddresses[networkName];
    assert( networkName in FertilizerTokenAddresses, "Need valid fertilizer address to interface with the contract");
    const fertilizerAddress: string = FertilizerTokenAddresses[networkName];
    try {
        await irrigationDiamond.WaterCommon_Initialize(beanStalkProxyAddress, fertilizerAddress, { gasLimit: 600000 });
    } catch (e) {
